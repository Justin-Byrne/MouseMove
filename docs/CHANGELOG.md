...
## [0.1.11] - 2023-09-11
### Added
- `MouseMove` class, private `setUserAgent ( )` function, to identify and set the present user agent

### Changed
- `Cursor` class
  - refactored private `getRangeValuesPoint ( )` to get the center point of the value of a range element
  - merged private `getRangeValuesPoint ( )` functions logic into `getCenterPoint ( )`
  - refactored public setters `distance ( )` and `angle ( )`, and utilities `nextElement ( )` and `setInteraction ( )`
  - modified private `getCenterPoint ( )` function, to account for vertical scrolling when requesting a client bounding rectangle; for an element

### Fixed
- `MouseMove` class
  - fixed how `generativeIndex` doesn't advance properly a select element is mirrored
  - fixed how `hotkeyListener ( )` fires the default hot-key's action when initiated

### Removed
- `Cursor` class, private `getValueFromStandardDeviation ( )` to get the standard deviation range value
...
| :-----: | :--------: | :----------------------------------------------------------------------: | :---------------------------------------------------------------- |
| 0.1.11  | 2023-09-14 | CURRENT REVISION                                                         | Minor refactoring and fixes for the Cursor and MouseMove classes
| 0.1.10  | 2023-09-11 | [165eab9](https://github.com/Justin-Byrne/MouseMove/commit/165eab9)      | Further refinements to the Cursor class, for range input elements
